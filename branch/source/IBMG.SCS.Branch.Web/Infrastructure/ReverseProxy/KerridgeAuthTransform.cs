    ...
    public void Apply(TransformBuilderContext context)
    {
        ...
        context.AddRequestTransform(async transformContext =>
        {
            ...
            if (path.StartsWith("/api/kerridge/", StringComparison.OrdinalIgnoreCase))
            {
                ...
                _logger.LogDebug("Rewrote path from {OriginalPath} to {NewPath}", path, newPath);
            }
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                _logger.LogDebug("Added Bearer token to Kerridge API request");
            }
            else
            {
                _logger.LogWarning("Failed to obtain JWT token for Kerridge API");
            }
        });
    }
